...
const editProductValidationsMiddleware = [
  body('name').notEmpty().withMessage('Debes escribir el nombre del producto').bail().isLength({ min: 3 }).withMessage('Mínimo 8 catacteres')
    .custom(async (value, { req }) => {
      try {
        ...
        if (existingProduct && value !== productId.name) {
  ...
  body('category').notEmpty().withMessage('Debes seleccionar la categoría del producto'),
...
module.exports = editProductValidationsMiddleware